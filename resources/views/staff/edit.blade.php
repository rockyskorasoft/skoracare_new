    ...
    <div class="col-md-12 divide-y-1 dashboard-card-main-col">
        <div class="card no-scale">
            <div class="card-body">
                ...
                @endif
                <form class="row g-3" action="{{ route('admin.staff.update', ['staff' => \App\Support\SecureRouteParameter::encode($staff->id)]) }}" method="post">
                    @csrf
                    @method('PUT')
                    
                    <x-input-field class="col-md-6" label="First Name" name="first_name" id="first_name"
                        type="text" :value="old('first_name', $staff->first_name)" placeholder="First Name"
                        errorField="first_name" labelClass="required" />
                    
                    <x-input-field class="col-md-6" label="Last Name" name="last_name" id="last_name"
                        type="text" :value="old('last_name', $staff->last_name)" placeholder="Last Name"
                        errorField="last_name" />
                    
                    <x-input-field class="col-md-6" label="Email Address" name="email" id="email"
                        type="email" :value="old('email', $staff->email)" placeholder="Email Address"
                        errorField="email" labelClass="required" />
                    
                    <x-input-field class="col-md-6" label="Phone Number" name="phone_no" id="phone_no"
                        type="number" :value="old('phone_no', $staff->phone_no)" placeholder="Phone Number"
                        errorField="phone_no" />
                    ...
                    <x-select-field class="col-md-6" label="Account Status" name="status" id="status"
                        ...
                        :value="old('status', $staff->status)" placeholder="{{ __('labels.select') }}"
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="role" id="role" class="form-select @error('role') is-invalid @enderror" required>
                            @foreach($roles as $r)
                                <option value="{{ $r->name }}" {{ old('role', $staffRoleName) == $r->name ? 'selected' : '' }}>
                                    {{ $r->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="doctor_id" id="doctor_id" class="form-select @error('doctor_id') is-invalid @enderror">
                                ...
                                @foreach($doctors as $doc)
                                    <option value="{{ $doc->id }}" {{ old('doctor_id', $staff->created_by) == $doc->id ? 'selected' : '' }}>
                                    ...
                                    <div class="col-md-4 clinic-item-col" data-doctor-id="{{ $clinic->doctor_id }}">
                                        <div class="form-check bg-white p-2 border rounded">
                                            <input type="checkbox" name="clinic_ids[]" value="{{ $clinic->id }}" id="clinic_chk_{{ $clinic->id }}" class="form-check-input ms-1 me-2 clinic-checkbox"
                                                {{ (is_array(old('clinic_ids', $assignedClinicIds)) && in_array($clinic->id, old('clinic_ids', $assignedClinicIds))) ? 'checked' : '' }}>
                                            ...
                                            <div class="col-md-3">
                                                <div class="form-check">
                                                    <input type="checkbox" name="permissions[]" value="{{ $p->name }}" id="perm_{{ $p->id }}" class="form-check-input"
                                                        {{ (is_array(old('permissions', $assignedPermissionNames)) && in_array($p->name, old('permissions', $assignedPermissionNames))) ? 'checked' : '' }}>
