@extends('layouts.app')
@section('title')
    {{ __('labels.create_title', ['action' => 'Clinic']) }}
@endsection
@section('content')
    <div class="d-flex gap-2 align-items-center justify-content-between mb-4 pb-2">
        <h3 class="page-title">{{ __('labels.create_title', ['action' => 'Clinic']) }}</h3>
    ...
    <div class="col-md-12 divide-y-1 dashboard-card-main-col">
        <div class="card no-scale">
            <div class="card-body">
                ...
                @endif
                <form class="row g-3" action="{{ route('admin.clinics.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    ...
                    <x-input-field class="col-md-6" label="Phone Number" name="phone_no" id="phone_no"
                        type="number" :value="old('phone_no')" placeholder="9971423449"
                        errorField="phone_no" />
