...
class UpdateRequest extends FormRequest
{
    ...
    {
        // Decode the route parameter to get the real DB id
        $encodedId = $this->route('landing_page');
        $realId    = \App\Support\SecureRouteParameter::decode($encodedId) ?? $encodedId;

        return [
            'clinic_name'              => 'required|string|max:255',
            'slug'                     => ['required', 'string', 'max:100', 'regex:/^[a-z0-9\-]+$/',
                                           Rule::unique('clinic_landing_pages', 'slug')->ignore($realId)],
